...
async function translate(language, content) {
  const response = await openai.chat.completions.create({
    ...
    messages: [{
      role: 'system',
      content: 'You will be provided with text as markdown file, and your task is to translate this text into requested language , and return result as markdown file'
    }, {
      role: 'user',
...
async function translateFileName(language, filename) {
  const response = await openai.chat.completions.create({
    ...
    messages: [{
      role: 'system',
      content: 'You will be provided with file name , and your task is to translate this text into requested language , and return result as filename with underscore instead of spaces between words.'
    }, {
      role: 'user',
